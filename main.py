...
이를 막기 위해 _parse_rows()에서 가격을 unit_sz로 나눠 "단위당 가격"으로 정규화한 값을
화면 전체(그래프·상태카드·가격표)에 사용하며, 원본 가격(raw_price)과 단위 정보는 참고용으로
함께 보관합니다.
...
def _parse_rows(raw_rows: list) -> list:
    ...
    for row in raw_rows:
        ...
        # 상자 단위) 원본 가격을 그대로 비교하면 "도매가 소매보다 비싸 보이는" 왜곡이
        # 생긴다. unit_sz로 나눠 "단위당(1kg 등) 가격"으로 정규화해 실제 비교가 되게 한다.
        unit = row.get("unit")
        unit_sz_raw = row.get("unit_sz")
        unit_size = _parse_unit_size(unit_sz_raw)
        unit_price = price / unit_size if unit_size else price
        ...
        parsed.append(
            {
                ...
                "division": division,
                "price": unit_price,  # 정규화된 단위당 가격 (그래프/분석에서 사용)
...
if fruit_df.empty or latest is None:
    ...
    with st.container(border=True):
        ...
        with st.expander("🧾 월별 가격표 보기"):
            ...
            def _typical_unit_label(div_name: str) -> str:
                """해당 구분(소매/도매)에서 가장 흔한 단위(unit·unit_sz)를 "1kg당" 형태로 요약한다.
                unit/unit_sz 컬럼이 없는 데모 데이터에서는 빈 문자열을 반환한다."""
                if "unit_sz" not in fruit_df.columns:
                    return ""
                sub = fruit_df[
                    (fruit_df["division"] == div_name) & (fruit_df["unit_sz"].astype(str) != "")
                ]
                if sub.empty:
                    return ""
                sz = sub["unit_sz"].mode().iloc[0]
                unit = sub["unit"].mode().iloc[0] if "unit" in sub.columns and not sub["unit"].mode().empty else ""
                return f" ({sz}{unit}당)" if sz else ""
    ...
    st.write("")
    unit_note = (
        "  ·  소매·도매 가격은 조사 단위(unit_sz)가 서로 달라(예: 소매 1kg / 도매 상자 단위) "
        "생기는 왜곡을 막기 위해 단위당 가격으로 환산해서 비교했어요."
