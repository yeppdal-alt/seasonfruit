...
소매/도매는 조사 단위(unit, unit_sz)가 서로 다를 수 있어(예: 소매 1kg vs 도매 10~15kg 상자)
원본 가격(pmm_avgprc)을 그대로 비교하면 도매가 소매보다 비싸 보이는 왜곡이 생길 수 있습니다.
이를 막기 위해 _parse_rows()/_unit_size_to_kg()에서 가능하면 무게(kg) 기준으로 통일해
"1kg당 가격"으로 환산한 값을 화면 전체(그래프·상태카드·가격표)에 사용합니다. 다만 개·상자
등 무게 단위가 아닌 경우는 억지로 환산하지 않고 원본 단위 그대로 두며(unit_basis="raw"),
원본 가격(raw_price)·단위 정보·환산 기준은 "🔍 원본 데이터로 확인" 표에서 참고용으로
그대로 볼 수 있습니다.
...
def _parse_unit_size(raw) -> float | None:
    """unit_sz(단위크기) 문자열에서 앞자리 숫자만 뽑아낸다. (예: "10kg" -> 10.0)"""
    if raw is None:
        return None
...
def _unit_size_to_kg(unit: str, size: float) -> float | None:
    """unit(단위 문자열)과 unit_sz(크기)를 kg 무게로 환산한다.

    "kg"/"키로"/"킬로" 계열이면 그대로, "g"/"그램" 계열이면 1000으로 나눠 kg로 바꾼다.
    "개"/"box"/"속" 등 무게가 아닌 단위는 그램·킬로 환산이 불가능하므로 None을 반환하고,
    호출부에서는 이 경우 kg 기준 비교 대상에서 제외한다(무리하게 억지 환산하지 않는다).
    """
    ...
    if "kg" in u or "키로" in u or "킬로" in u:
        return size
    if "g" in u or "그램" in u:  # "kg"는 위에서 먼저 걸러졌으므로 여기 도달하면 순수 g 단위
        return size / 1000.0
    return None  # 개/속/box 등 무게가 아닌 단위 - kg로 억지 환산하지 않는다
...
def _parse_rows(raw_rows: list) -> list:
    ...
    for row in raw_rows:
        ...
        # 소매/도매는 조사 단위(unit, unit_sz)가 서로 달라(예: 소매는 1kg, 도매는 10~15kg
        # 상자 단위) 원본 가격을 그대로 비교하면 "도매가 소매보다 비싸 보이는" 왜곡이 생긴다.
        # 가능하면 kg 무게 기준으로 통일해서 "1kg당 가격"으로 비교하고, 개/상자처럼 무게로
        # 환산할 수 없는 단위는 억지로 맞추지 않고 unit_sz 숫자로만 나눈 값을 쓰되
        # unit_basis="raw"로 표시해 화면에서 구분할 수 있게 한다.
        unit = row.get("unit")
        unit_sz_raw = row.get("unit_sz")
        unit_size = _parse_unit_size(unit_sz_raw)
        weight_kg = _unit_size_to_kg(unit, unit_size) if unit_size else None
        if weight_kg:
            unit_price = price / weight_kg  # 1kg당 가격
            unit_basis = "kg"
        elif unit_size:
            unit_price = price / unit_size  # 단위 개수로만 나눈 값 (무게 환산 불가)
            unit_basis = "raw"
        else:
            unit_price = price
            unit_basis = "raw"
        ...
        parsed.append(
            {
                ...
                "division": division,
                "price": unit_price,  # 정규화된 가격 (kg 기준이면 1kg당, 아니면 원본 단위당)
                "unit_basis": unit_basis,  # "kg" = 무게로 정확히 환산됨 / "raw" = 환산 불가
                "raw_price": price,  # 원본 조사 가격 (참고/검증용)
                "unit": str(unit).strip() if unit not in (None, "") else "",
                "unit_sz": str(unit_sz_raw).strip() if unit_sz_raw not in (None, "") else "",
            }
        )
    return parsed
...
if fruit_df.empty or latest is None:
    ...
    # 연간 가격 추이 (Plotly)
    # ------------------------------------------------------------------
    _chart_month_count = len(overall_monthly)
    st.markdown(f"#### 📈 최근 {_chart_month_count}개월 가격 추이")
    ...
    with st.container(border=True):
        ...
        with st.expander("🧾 월별 가격표 보기"):
            ...
            def _typical_unit_label(div_name: str) -> str:
                """해당 구분(소매/도매)의 가격이 실제로 "1kg당"으로 환산됐는지 요약해 헤더에 표시한다.
                (kg/g 등 무게 단위가 아니라 개·상자 단위라 무게 환산이 불가능했다면 그 사실을
                숨기지 않고 "환산 불가"로 그대로 알려준다.) unit_basis 컬럼이 없는 데모 데이터에서는
                빈 문자열을 반환한다."""
                if "unit_basis" not in fruit_df.columns:
                    return ""
                sub = fruit_df[fruit_df["division"] == div_name]
                if sub.empty:
                    return ""
                if (sub["unit_basis"] == "kg").mean() >= 0.5:
                    return " (1kg당)"
                return " (단위 환산 불가)"
            ...
            if "소매" in divisions or "도매" in divisions:
                for div_name in ["소매", "도매"]:
                    if div_name in divisions:
                        ...
                        # 소매/도매는 조사 단위(unit_sz)가 서로 달라 원본 가격을 그대로 비교하면
                        # 도매가 더 비싸 보이는 왜곡이 생길 수 있어, 이미 단위당 가격으로 정규화된
                        # 값을 사용하고(컬럼 "price"), 어떤 기준으로 환산했는지 헤더에 표시한다.
                        col_name = div_name + _typical_unit_label(div_name)
    ...
    st.write("")
    unit_note = (
        "  ·  소매·도매 가격은 가능한 경우 1kg당 가격으로 통일해서 비교했어요(개·상자처럼 "
        "무게 단위가 아닌 경우는 환산이 어려워 원본 단위를 그대로 썼어요 - 아래 '원본 데이터로 "
        "확인'에서 항목별로 볼 수 있어요)."
        if is_live and "unit_basis" in fruit_df.columns
        else ""
    )
    st.caption(
